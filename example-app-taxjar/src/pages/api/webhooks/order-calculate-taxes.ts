...
type CalculateTaxesPayload = Extract<
  CalculateTaxesEventFragment,
...
export const orderCalculateTaxesSyncWebhook =
  new SaleorSyncWebhook<CalculateTaxesPayload>({
...
export default orderCalculateTaxesSyncWebhook.createHandler(
  async (req, res, ctx) => {
    ...
    try {
      ...
      const calculatedTaxes = await activeConnectionService.calculateTaxes(
        payload
      );
