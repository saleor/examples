...
const Header = () => {
  return (
    <Section.Header>
      Match Saleor tax classes to TaxJar tax categories
    </Section.Header>
  );
};
      ...
      description={
        <>
          <Text display="block" as="span" marginBottom={4}>
            To extend the base tax rate of your products, you can map Saleor tax
            classes to TaxJar tax categories.
          </Text>
          <Text display="block" as="span" marginBottom={4}>
            This way, the product&apos;s Saleor tax class will be used to
            determine the TaxJar tax code needed to calculate the tax rate.
          </Text>
          <Text as="p" marginBottom={4}>
            If you haven&apos;t created any tax classes yet, you can do it in
            the{" "}
            <AppDashboardLink
...
const TaxJarMatcher = () => {
  ...
  const { isLoading } = trpcClient.taxJarConnection.verifyConnections.useQuery(
    undefined,
    {
      onError: () => {
